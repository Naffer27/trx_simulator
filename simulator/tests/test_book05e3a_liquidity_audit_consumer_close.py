"""
BOOK-05e.3a — Liquidity Engine Audit Trail: close-time integration in
TradingConsumer._db_close_position_atomic().

Covers the single integration point authorized for this block: a new,
independent block placed immediately after BOOK-05d.3a's own nested
...
(BOOK-05d.2, untouched) returns a non-None instance. Never touches
tasks.py or admin.py (BOOK-05e.3b/3c, not started).
"""
...
class LiquidityLedgerAuditCloseIntegrationTests(TestCase):
    ...
    def test_success_creates_exactly_one_event(self):
        ...
        self.assertEqual(event.metadata["close_reason"], "manual")
    ...
    # ── 11. LiquidityDecision/LiquidityLedger no modificados ────────────
    ...
    # ── 12. BrokerLedger sin escrituras adicionales ─────────────────────
    ...
    # ── 13. No se crean eventos LIQUIDITY extra ─────────────────────────
