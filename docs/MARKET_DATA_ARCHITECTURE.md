...
## 14. MD-5 — Decisión de Fuente de Verdad del Instrumento (FASE 0 + FASE 1)

**FASE 0 (RFC, 2026-07-28, solo análisis — sin código):** confirmó con evidencia ejecutada (`python manage.py audit_instrument_profiles`) que el drift real entre `SymbolSpec` e `Instrument` es prácticamente nulo hoy (15 símbolos: 12 `MATCH`, 3 `DRIFT` — los tres únicamente cosméticos, `display_name`, cero diferencias críticas, cero símbolos huérfanos en cualquiera de los dos lados). Con esa evidencia, se descartó migrar a DB-first: el costo (resolver el límite de un-solo-proveedor de `Instrument` frente a los hasta 3 proveedores que `SymbolSpec` puede declarar, y decidir qué hacer con `base_price`/`sim_drift`, conceptos exclusivos de fallback simulado sin equivalente en `Instrument`) no se justifica frente a un beneficio (panel de staff sin deploy) que hoy nadie ha solicitado operacionalmente.

**Decisión (Opción B modificada — "code-first con DB de configuración administrativa"):** `SymbolSpec` sigue siendo, sin cambios, la única fuente de verdad del runtime de ejecución. `Instrument` se redefine explícitamente como catálogo de referencia administrativa — legítimo para planificación/comparación, sin ningún efecto sobre trading, pricing, margen o riesgo. Esta decisión no requirió ni requiere ningún cambio a `market_data/symbol_specs.py`, a `simulator/models.py::Instrument`, ni a ninguno de los ~20 call sites reales de `get_spec()` (`feeds.py`, `consumers.py`, `broker_risk.py`, `broker_exposure.py`, `pnl_engine.py`, `pricing_context.py`, `spread_engine.py`, `risk_engine.py`, entre otros).

**FASE 1 (implementación, 2026-07-28) — cierre del riesgo operacional silencioso, sin tocar el runtime:**

- **`simulator/admin.py::InstrumentAdmin`** — banner de advertencia (`messages.warning()`, Django admin, visible en cada carga de la lista, alta y edición) dejando explícito que ningún campo editado aquí tiene efecto real. `readonly_fields` se revisó y se dejó sin cambios (`created_at`/`updated_at` únicamente) — `Instrument` sigue siendo un catálogo de referencia legítimamente editable (comparación de drift, planificación futura); el riesgo identificado en el RFC era de **visibilidad**, no de que la edición en sí debiera restringirse.
- **`MARKET_DATA_CATALOG_DRIFT_CHECK_ENABLED`** — default cambiado a `True` en desarrollo (`.env.example`) y en staging (`deploy/.env.staging.template`); el fallback hardcodeado en `trx_simulator/settings.py` permanece `False` (sin cambios), de modo que cualquier entorno que no fije explícitamente la variable — incluida una futura producción — sigue partiendo del valor seguro. Este flag sigue **sin ningún llamador nuevo en el runtime**: `check_runtime_catalog_drift()` (FOUNDATION-12) permanece invocable únicamente bajo demanda (`python manage.py audit_instrument_profiles`, o el propio helper si algo lo llama en el futuro) — no se agregó ninguna tarea de Celery Beat ni ningún hook nuevo en `feeds.py`/`consumers.py`/`tasks.py` en esta fase (decisión explícita: mantener el radio de cambio fuera de cualquier call site del runtime).

**Qué NO cambió en FASE 1 (verificado):** `market_data/symbol_specs.py`, `simulator/models.py::Instrument` como fuente de ejecución, `get_spec()`, `broker_risk.py`, `feeds.py`, `consumers.py`, `pricing_context.py`, `risk_engine.py` — ninguno fue tocado. Ningún caller nuevo de `get_runtime_instrument()`/`check_runtime_catalog_drift()` fue agregado al runtime.

**Próximos pasos posibles (no decididos, no iniciados):** MD-5 FASE 2 (tabla real `Instrument`→`ProviderMapping` uno-a-muchos, reemplazando el overlay manual `_KNOWN_SECONDARY_PROVIDERS`); MD-5 FASE 3 (revisitar DB-first) explícitamente **no recomendada** salvo que surja una necesidad operacional concreta que hoy no existe.
