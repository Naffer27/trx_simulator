...
# ─────────────────────────────────────────────────────────────────────────
# BROKER-ECONOMICS-02C — Owner Control Plane: Broker Economic Adjustment.
# The first Owner-only user-facing surface in this application. Two
# steps: preview (read-only, generates the idempotency_key exactly once)
# and confirm (TOTP-gated, calls owner_actions.owner_broker_economic_
# adjustment() exclusively — never writes BrokerLedger/
# BrokerEconomicAdjustment itself, never re-generates the key).
# ─────────────────────────────────────────────────────────────────────────

@login_required
@rate_limit("owner_broker_adjustment_preview", limit=20, window=300, by="user")
def owner_broker_adjustment_view(request):
    """
    PREVIEW step. GET renders a blank form. POST resolves and validates
    the submitted references (read-only) and renders the CONFIRM step
    with a freshly generated, cryptographically unpredictable
    idempotency_key embedded as a hidden field — generated exactly once,
    here, never regenerated by the confirm step (BROKER-ECONOMICS-02C's
    closed idempotency design). Creates NOTHING: no BrokerLedger row, no
    BrokerEconomicAdjustment row, no Wallet/Treasury/IB/TradingAccount
    write of any kind — every DB access below is a plain .filter().first()
    read or the existing, read-only calculate_broker_pnl().
    """
    from django.core.exceptions import PermissionDenied

    from .permission_levels import is_owner_root

    if not is_owner_root(request.user):
        raise PermissionDenied("Owner Root access required.")

    if request.method != "POST":
        return render(request, "simulator/owner_broker_adjustment_form.html", {})

    from .broker_pnl import calculate_broker_pnl
    from .models import BrokerEconomicAdjustment, BrokerLedger, TradingAccount

    mode = request.POST.get("mode", "new")
    reason = (request.POST.get("reason") or "").strip()
    symbol = (request.POST.get("symbol") or "").strip() or None
    source_ledger_id = (request.POST.get("source_ledger_id") or "").strip() or None
    source_account_id = (request.POST.get("source_account_id") or "").strip() or None
    reverses_id = (request.POST.get("reverses_id") or "").strip() or None
    raw_amount_input = request.POST.get("amount", "")
    ...
    if source_ledger_id:
        ...
        if source_ledger is None:
            errors.append(f"BrokerLedger #{source_ledger_id} no existe.")

    source_account = None
    if source_account_id:
        source_account = TradingAccount.objects.filter(pk=source_account_id).first()
        if source_account is None:
            errors.append(f"TradingAccount #{source_account_id} no existe.")

    reverses = None
    amount = None
    if mode == "reversal":
        if not reverses_id:
            errors.append("Debes indicar el ajuste a revertir.")
        else:
            reverses = BrokerEconomicAdjustment.objects.filter(pk=reverses_id).first()
            if reverses is None:
                errors.append(f"BrokerEconomicAdjustment #{reverses_id} no existe.")
            else:
                # Server-derived — shown to the Owner, never an editable
                # field. The confirm step re-derives this independently
                # and never reads a client-supplied reversal amount.
                amount = -reverses.amount
    else:
        try:
            amount = Decimal(raw_amount_input.strip())
        except (InvalidOperation, ValueError, AttributeError):
            errors.append("El monto no es un valor decimal válido.")
        else:
            if amount == 0:
    ...
    if errors:
        return render(request, "simulator/owner_broker_adjustment_form.html", {
            "errors": errors, "mode": mode, "reason": reason, "symbol": symbol or "",
            "source_ledger_id": source_ledger_id or "",
            "source_account_id": source_account_id or "",
            "reverses_id": reverses_id or "", "amount_input": raw_amount_input,
        })

    pnl_preview = None
    if source_account is not None:
        pnl_preview = calculate_broker_pnl(account_id=source_account.id)

    idempotency_key = _secrets.token_urlsafe(32)  # generated exactly ONCE

    return render(request, "simulator/owner_broker_adjustment_confirm.html", {
        "mode": mode, "amount": amount, "reason": reason, "symbol": symbol or "",
        "source_ledger": source_ledger, "source_account": source_account,
        "reverses": reverses, "pnl_preview": pnl_preview,
...
@login_required
@require_POST
@rate_limit("owner_broker_adjustment_confirm", limit=10, window=300, by="user")
def owner_broker_adjustment_confirm_view(request):
    """
    CONFIRM step. TOTP required. Reads idempotency_key from the POST
    body verbatim — the SAME hidden-field value the preview step
    generated — and NEVER generates a new one here. Calls
    owner_actions.owner_broker_economic_adjustment() exclusively; this
    view never constructs a BrokerLedger or BrokerEconomicAdjustment row
    itself. For a reversal, the client's `amount` field (if any) is
    never even read — the wrapper derives it server-side.
    """
    from django.core.exceptions import PermissionDenied

    from .owner_actions import InvalidAdjustment, owner_broker_economic_adjustment
    from .permission_levels import is_owner_root
    ...
    amount = None
    if mode != "reversal":
        # Only read for a non-reversal adjustment. For "reversal" mode,
        # any client-supplied amount is deliberately never extracted
        # here at all — owner_broker_economic_adjustment() derives it
        # itself from reverses_id.
        amount = request.POST.get("amount", "")

    if not idempotency_key:
        return render(request, "simulator/owner_broker_adjustment_form.html", {
            "errors": ["Falta el idempotency_key del preview — vuelve a generar la vista previa."],
            "mode": mode, "reason": reason, "symbol": symbol or "",
            "source_ledger_id": source_ledger_id or "",
            "source_account_id": source_account_id or "",
            "reverses_id": reverses_id or "", "amount_input": amount or "",
        })

    try:
        adjustment = owner_broker_economic_adjustment(
            amount=amount, reason=reason, actor=request.user,
            totp_code=totp_code, idempotency_key=idempotency_key,
            source_ledger_id=source_ledger_id, source_account_id=source_account_id,
            symbol=symbol, reverses_id=reverses_id,
        )
    except PermissionDenied:
        raise
    except InvalidAdjustment as exc:
        return render(request, "simulator/owner_broker_adjustment_form.html", {
            "errors": [str(exc)], "mode": mode, "reason": reason, "symbol": symbol or "",
            "source_ledger_id": source_ledger_id or "",
            "source_account_id": source_account_id or "",
            "reverses_id": reverses_id or "", "amount_input": amount or "",
        })

    return render(request, "simulator/owner_broker_adjustment_result.html", {
