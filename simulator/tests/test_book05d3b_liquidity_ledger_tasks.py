"""
BOOK-05d.3b — Liquidity Ledger integration into
...
negative balance guard / pos.delete(), inside this function's own
_tx.atomic() (local alias for django.db.transaction). No test here
touches TradingConsumer or admin.py — those are BOOK-05d.3a (done) and
BOOK-05d.3c (not started), out of scope here.
"""
...
class LiquidityLedgerTasksIntegrationTests(TestCase):
    ...
    def test_with_liquidity_decision_creates_exactly_one_entry(self):
        ...
        self.assertEqual(entry.meta, {"trader_pnl": 10.0, "close_reason": "daemon_stopout"})
    ...
    def test_lookup_database_error_is_contained_close_completes_normally(self):
        ...
        LiquidityDecision lookup must never leave this function's own
        outer _tx.atomic() unusable — if it did, pos.delete() and the
        balance update below would raise TransactionManagementError
