...
def broker_economics_summary(
    *, period: str = PERIOD_LIFETIME, start=None, end=None,
    ...
    # (provider_cost_economics.py's booking gate). The figure below
    # reflects exactly what has been booked in the queried scope — it
    # can read $0.00 when no qualifying ACTUAL cost exists yet there, or
    # a real non-zero amount the moment one is captured and posted (the
    # 04C.5 manual acceptance run confirmed both states). Absence of fee
    # evidence is never read as a confirmed zero cost — it is simply the
    # current state of the evidence this module was given.
    ...
    retained = RetainedEconomicsSummary(
        ...
        coverage=CategoryCoverage(
            ...
            "cost (BROKER-ECONOMICS-04C.4) is sourced from BrokerLedger.REV_PROVIDER_COST — "
            "ACTUAL + FINAL + USD-valued only, per provider_cost_economics.py's booking gate. "
            "This figure reflects exactly what has been booked in the queried scope — it can "
            "read $0.00 when no qualifying provider cost exists there yet, or a real non-zero "
            "amount once one is captured and posted; absence of fee evidence is never presented "
            "as a confirmed zero cost. This figure must never be presented as final/true net profit.",
        ),
    )
    ...
    return BrokerEconomicsSummary(
        ...
        provider_cost_coverage=CategoryCoverage(
            ...
            "forward-only. This sum reflects exactly what has been booked in the queried scope "
            "— it reads $0.00 whenever no qualifying ACTUAL cost has been captured there yet, "
            "and a real non-zero amount the moment one is (the 04C.5 manual acceptance run "
            "confirmed both states end to end). ESTIMATED/UNKNOWN provider costs are captured "
            "in ProviderCostRecord for audit but never included here. That the certified "
            "NowPayments adapter (provider_cost_adapters.py) has so far observed no `fee` "
            "evidence in the real payments checked (04C.2: 0/4 live) is a historical, "
            "evidential observation about that one rail at that point in time — never a "
            "permanent economic constant of this figure. PARTIAL reflects that "
